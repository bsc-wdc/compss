...
public class CExecutor extends ExternalExecutor {
    ...
    public ArrayList<String> getTaskExecutionCommand(NIOWorker nw, NIOTask nt, String sandBox, int[] assignedCoreUnits, int[] assignedGPUs) {
        ...
        if (nt.getResourceDescription().getProcessors().size() > 1) logger.debug("mida interna de la memoria de la gpu-2: " + nt.getResourceDescription().getProcessors().get(1).getInternalMemory());
