...
public class DefaultSchedulerPolicies extends SchedulerPolicies {
	...
	private HashMap<Integer, PSCOId> idToPscoId = new HashMap<Integer, PSCOId>();
                ...
                for (Parameter p : params) {
                    if (p instanceof DependencyParameter) {
						...
						if (type == ParamType.SCO_T) {
							...
							PSCOId pscoId = this.idToPscoId.get(scop.getCode());
							if (pscoId != null) {
								scop.setType(ParamType.PSCO_T);
								scop.setValue(pscoId);
								logger.info("SCO change to PSCO with id " + pscoId);
							}
						}					
						...
						if (p.getType() == ParamType.PSCO_T) {
							...
							switch (scop.getDirection()) {
							case IN:
								DataAccessId.RAccessId raId = (DataAccessId.RAccessId) scop
										.getDataAccessId();
								dId = raId.getReadDataInstance();
								break;
							case INOUT:
								DataAccessId.RWAccessId rwaId = (DataAccessId.RWAccessId) scop
										.getDataAccessId();
							...
							if (dId != null) {
								List<String> backends = getPSCOLocations((SCOParameter) p);
								for (String backendID : backends) {
									if (backendID.compareTo(host.getName()) == 0) {
										score++;
									}
								}
                        ...
                        if (p instanceof DependencyParameter) {
							...
							if (p.getType() == ParamType.PSCO_T) {
								...
								if (dId != null) {
									List<String> backends = getPSCOLocations((SCOParameter) p);
									for (String backendID : backends) {
										if (backendID.equals(ownerName)) {
											score--;
											break;
										}
										if (backendID.equals(destResource)) {
											score += 2;
											break;
										}
									}
	...
	public List<String> getPSCOLocations(SCOParameter p) {
		...
		if (!(p.getValue() instanceof PSCOId)) {
			...
			PSCOId pscoId = this.idToPscoId.remove(id);
			p.setValue(pscoId);
		}

		if ((p.getValue() instanceof PSCOId)) {
			String pscoId = ((PSCOId) (p.getValue())).getId();
			logger.info("Getting backends for PSCO " + pscoId + " with hascode " + id);
			try {
				if (tracing) {
			...
			logger.info("Backends for PSCO " + pscoId + ": " + backends);
		}
	...
	@Override
	public HashMap<Integer, PSCOId> getIdToPscoId() {
		return idToPscoId;
	}
}
