...
// Workflow methods
// ******************************
long SOCKET_WF_getId(CompssWorkflow* self) {
    return 0;
}
...
void SOCKET_WF_openTaskGroup(CompssWorkflow* self, const char* groupName, bool implicitBarrier) {
    ...
    socket_send_command(build_open_task_group_command(groupName, implicitBarrier));
    debug_printf("[BINDING-COMMONS] - @SOCKET_OpenTaskGroup - COMPSs group name: %s\n", groupName);
}

void SOCKET_WF_closeTaskGroup(CompssWorkflow* self, const char* groupName) {
    debug_printf("[BINDING-COMMONS] - @SOCKET_CloseTaskGroup - COMPSs group name: %s\n", groupName);
    socket_send_command(build_close_task_group_command(groupName));
    debug_printf("[BINDING-COMMONS] - @SOCKET_CloseTaskGroup - Task group %s closed.\n", groupName);
}

void SOCKET_WF_executeTask(CompssWorkflow* self, char* signature, int onFailure, int timeout, int priority, int numNodes, int reduce, int reduceChunkSize,
...
void SOCKET_WF_executeHttpTask(CompssWorkflow* self, char* signature, int onFailure, int timeout, int priority, int numNodes, int reduce,
...
void SOCKET_WF_cancelTaskGroup(CompssWorkflow* self, const char* groupName, char** exceptionMessage) {
    debug_printf("[BINDING-COMMONS] - @SOCKET_CancelTaskGroup - COMPSs group name: %s\n", groupName);
    socket_send_command(build_cancel_task_group_command(groupName));
...
void SOCKET_WF_cancelApplicationTasks(CompssWorkflow* self) {
    debug_printf("[BINDING-COMMONS] - @SOCKET_Cancel_Application_Tasks\n");
    socket_send_command(build_cancel_application_tasks_command());
    debug_printf("[BINDING-COMMONS] - @SOCKET_Cancel_Application_Tasks - Tasks cancelled\n");
}

void SOCKET_WF_barrier(CompssWorkflow* self) {
    debug_printf("[BINDING-COMMONS] - @SOCKET_Barrier - Waiting tasks\n");
    socket_send_command(build_barrier_command());
    socket_read_line();
    debug_printf("[BINDING-COMMONS] - @SOCKET_Barrier\n");
}

void SOCKET_WF_barrierWithFlag(CompssWorkflow* self, bool noMoreTasksFlag) {
    debug_printf("[BINDING-COMMONS] - @SOCKET_Barrier - Waiting tasks\n");
    socket_send_command(build_barrier_new_command(noMoreTasksFlag));
    socket_read_line();
    debug_printf("[BINDING-COMMONS] - @SOCKET_Barrier\n");

}

void SOCKET_WF_barrierGroup(CompssWorkflow* self, const char* groupName, char** exceptionMessage) {
    debug_printf("[BINDING-COMMONS] - @SOCKET_BarrierGroup - COMPSs group name: %s\n", groupName);
    socket_send_command(build_barrier_group_command(groupName));
    bool barrier_finished = false;
    while (!barrier_finished) {
        std::string result = socket_read_line();
        const char* buf = result.c_str();
...
void SOCKET_WF_noMoreTasks(CompssWorkflow* self) {
    debug_printf ("[BINDING-COMMONS] - @SOCKET_WF_noMoreTasks\n");
    debug_printf ("[BINDING-COMMONS] NOT YET IMPLEMENTED");
}


int SOCKET_WF_isFileAccessed(CompssWorkflow* self, char* fileName){
    debug_printf("[BINDING-COMMONS] - @SOCKET_Accessed_File - Calling runtime isFileAccessed method  for %s  ...\n",
                 fileName);
    socket_send_command(build_file_accessed_command(fileName));
    std::string result = socket_read_line();
...
void SOCKET_WF_openFile(CompssWorkflow* self, char* fileName, int mode, char** buf) {
debug_printf("[BINDING-COMMONS] - @SOCKET_Open_File - Calling runtime OpenFile method  for %s and mode %d ...\n",
                 fileName,
                 mode);
    socket_send_command(build_open_file_command(fileName, mode));
    std::string result = socket_read_line();
    const char* response = response_payload(result);
...
void SOCKET_WF_getFile(CompssWorkflow* self, char* fileName) {
    debug_printf("[BINDING-COMMONS] - @SOCKET_Get_File - Calling runtime getFile method...\n");
    socket_send_command(build_get_file_command(fileName));
    std::string result = socket_read_line();
    const char* response = response_payload(result);
...
void SOCKET_WF_closeFile(CompssWorkflow* self, char* fileName, int mode) {
    debug_printf("[BINDING-COMMONS] - @SOCKET_Close_File - Calling runtime closeFile method...\n");
    socket_send_command(build_close_file_command(fileName, mode));
    debug_printf("[BINDING-COMMONS] - @SOCKET_Close_File - COMPSs filename: %s\n", fileName);
}


bool SOCKET_WF_deleteFile(CompssWorkflow* self, char* fileName, int wait, int applicationDelete) {
    debug_printf("[BINDING-COMMONS] - @SOCKET_Delete_File - Calling runtime deleteFile method...\n");
    socket_send_command(build_delete_file_command(fileName, wait, applicationDelete));
    std::string result = socket_read_line();
    const char* response = response_payload(result);
    int res = atoi(response);
...
void SOCKET_WF_getDirectory(CompssWorkflow* self, char* dirName) {
    debug_printf("[BINDING-COMMONS] - @SOCKET_Get_Directory - Calling runtime getDirectory method...\n");
    socket_send_command(build_get_directory_command(dirName));
    std::string result = socket_read_line();
    const char* response = response_payload(result);
    int res = atoi(response);
    (void)res;
...
void SOCKET_WF_getObject(CompssWorkflow* self, char* objectId, char** buf) {
    debug_printf("[BINDING-COMMONS] - @SOCKET_Get_Object - Calling runtime getObject method...\n");
    socket_send_command(build_get_object_command(objectId));
    std::string result = socket_read_line();
    const char* response = response_payload(result);
    *buf = strdup(response ? response : "");
...
void SOCKET_WF_deleteObject(CompssWorkflow* self, char* objectId, int** buf) {
    debug_printf("[BINDING-COMMONS] - @SOCKET_Delete_Object - Calling runtime deleteObject method...\n");
    socket_send_command(build_delete_object_command(objectId));
    std::string result = socket_read_line();
    const char* response = response_payload(result);
    int res = atoi(response);
...
void SOCKET_WF_snapshot(CompssWorkflow* self) {
    debug_printf("[BINDING-COMMONS] - @SOCKET_Snapshot - Snapshot \n");
    socket_send_command(build_snapshot_command());
    socket_read_line();
    debug_printf("[BINDING-COMMONS] - @SOCKET_Snapshot\n");

}



CompssWorkflow* SOCKET_RegisterWorkflow() {
    debug_printf("[BINDING-COMMONS] - @SOCKET_RegisterWorkflow\n");
    ...
    wf->getId = SOCKET_WF_getId;
    wf->deregister = SOCKET_WF_deregister;
    ...
    wf->noMoreTasks = SOCKET_WF_noMoreTasks;
    wf->barrier = SOCKET_WF_barrier;
    wf->barrierWithFlag = SOCKET_WF_barrierWithFlag;
    wf->barrierGroup = SOCKET_WF_barrierGroup;
    wf->snapshot = SOCKET_WF_snapshot;
    wf->get_object = SOCKET_WF_getObject;
    wf->delete_object = SOCKET_WF_deleteObject;
    wf->is_file_accessed = SOCKET_WF_isFileAccessed;
    wf->open_file = SOCKET_WF_openFile;
    wf->get_file = SOCKET_WF_getFile;
    wf->close_file = SOCKET_WF_closeFile;
    wf->delete_file = SOCKET_WF_deleteFile;
    wf->get_directory = SOCKET_WF_getDirectory;

    debug_printf ("[BINDING-COMMONS] - @SOCKET_RegisterWorkflow - Workflow registered\n");
    return wf;
}
