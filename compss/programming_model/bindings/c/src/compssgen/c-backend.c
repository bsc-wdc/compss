...
static void generate_remove_and_serialize_case(FILE *outFile, Types current_types)
{
  ...
  for (i=0; i< current_types.num; i++){
	...
	fprintf(outFile, "\t\t\t %s *data_%d;", dataType, i);
  ...
  fprintf(outFile, "int serializeData(string id, char* filename, std::map<std::string, void*>& cache, std::map<std::string, int>& types)\n");
  fprintf(outFile, "{\n");
  fprintf(outFile, "\n");
