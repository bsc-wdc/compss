...
public abstract class Job<T extends COMPSsWorker> {
	...
	private static final String classpathFromEnvironment = (System.getProperty(ITConstants.IT_WORKER_CP) != null && !System.getProperty(
			ITConstants.IT_WORKER_CP).equals("")) ? System.getProperty(ITConstants.IT_WORKER_CP) : "\"\"";
	private final String classpathFromFile;
	private final String workerClasspath;

	private static final String pythonpathFromEnvironment = (System.getProperty(ITConstants.IT_WORKER_PP) != null && !System.getProperty(
			ITConstants.IT_WORKER_PP).equals("")) ? System.getProperty(ITConstants.IT_WORKER_PP) : "\"\"";
	...
	public Job(int taskId, TaskParams task, Implementation<?> impl, Resource res, JobListener listener) {
		 ...
		 */
		/*
		 * This variables are only used by GAT since NIO loads them from the
		 * worker rather than specific variables per job
		 */
