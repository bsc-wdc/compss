...
public class NIOWorker extends NIOAgent {
	...
	public NIOWorker(String uuid, int snd, int rcv, int masterPort, String hostName) {
		...
		/*
		 * tasks = new HashMap<Integer, NIOTask>(); jobsPending = new
		 * HashMap<Integer, LinkedList<String>>(); transferToJobs = new
		...
		pool = new ThreadPool(jobThreads, POOL_NAME, new JobLauncher(jobQueue,
				this, NIOWorker.wLogger, NIOWorker.workerDebug));
		try {
			pool.startThreads();
		} catch (Exception e) {
	...
	public static void main(String[] args) {
		...
		// Wait for the Transfer Manager thread to finish (the shutdown is
		// received on that thread)
		try {
			tm.join();
