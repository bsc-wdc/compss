...
public class ThreadBinderCPU implements ThreadBinder {
    ...
    private ArrayList<ArrayList<Integer>> computingUnitsIds = new ArrayList<ArrayList<Integer>>();
    private ArrayList<ArrayList<Integer>> bindedComputingUnits = new ArrayList<ArrayList<Integer>>();
    private ArrayList<Integer> availableSlots = new ArrayList<Integer>();
    int numThreads;
    
    /**i
     * @param numThreads               : amount of tasks to be launched in a given node
     * @throws InitializationException
     * It is important to realize that NUMA node information is expected to be of type lowerBound-upperBound. 
     * In case the information is given in a different format (for example, lowerBound1-upperBound1, lowerBound2-upperBound2) the second interval won't be taken in account
     * In case several lines references different NUMANodes (a single node has several CPU intervals), the system will consider each line as a different NUMANode
     */
        ...
        try {
            ...
            while ((s = stdOutput.readLine()) != null) {
                if (s.contains("NUMA") && !s.contains("NUMA node(s)")) {
                    ...
                    ArrayList<Integer> currentBounds = new ArrayList<Integer>();
     ...
     * @param sockets : maximum different amount of sockets allowed to host the threads
    ...
    public int[] bindComputingUnits(int jobId, int numCUs) throws UnsufficientAvailableComputingUnitsException {
        ...
        synchronized (this.bindedComputingUnits) {
            /*if (debug) {
                StringBuilder sb = new StringBuilder("[ThreadBinderCPUs] Available slots for task " + jobId + ": ");
                sb.append(this.availableSlots.get(0));
                for (int i = 1; i < this.availableSlots.size(); ++i){
                    sb.append(" " + this.availableSlots.get(i));
                }
                logger.debug(sb.toString());
            }*/
