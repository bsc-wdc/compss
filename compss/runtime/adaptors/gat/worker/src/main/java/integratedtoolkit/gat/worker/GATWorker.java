...
public class GATWorker {
	 ...
	 * for Write. Invokes the desired method by reflection and serializes all
	 * the objects that has been modified and the result.
	 *
	 * @param args
	 *            args for the execution: arg[0]: boolean enable debug arg[1]:
	 *            String implementing core class name arg[2]: String core method
	 *            name arg[3]: boolean is the method executed on a certain
	 *            instance arg[4]: integer amount of parameters of the method
	 *            arg[5+]: parameters of the method For each parameter: type:
	 *            0-10 (file, boolean, char, string, byte, short, int, long,
	 *            float, double, object) [substrings: amount of substrings (only
	 *            used when the type is string)] value: value for the parameter
	 *            or the file where it is contained (for objects and files)
	...
	public static void main(String args[]) {
		...
		Object values[];
		if (hasTarget) {
			// The target object of the last parameter before the return value
			// (if any)
		...
		for (int i = 0; i < numParams; i++) {
			// We need to use wrapper classes for basic types, reflection will
		...
		// Check if all the output files have been actually created (in case
		// user has forgotten)
		// No need to distinguish between IN or OUT files, because IN files will
		// exist, and if there's one or more missing,
