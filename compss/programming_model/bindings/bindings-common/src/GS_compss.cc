...
void GS_ExecuteHttpTask(long appId, char* methodType, char* baseUrl, char* signature, char* onFailure, int timeout, int priority, int numNodes, int reduce, int reduceChunkSize,
                        int replicated, int distributed, int hasTarget, int numReturns, int numParams, void** params) {
    JNI_ExecuteHttpTask(appId, methodType, baseUrl, signature, onFailure, timeout, priority, numNodes, reduce, reduceChunkSize, replicated, distributed, hasTarget, numReturns, numParams, params);
}
