		...
		if (state.getUseCloud()) {
			...
			for (ResourceCreationRequest rcr : CloudManager.getPendingRequests()) {
				...
				for (int coreId = 0; coreId < simTasks.length; coreId++) {
					...
					state.updateHostInfo(rcr.getRequested().getName(), rcr.getRequested().getType(), rcr.getRequested()
							.getTotalComputingUnits(), rcr.getRequested().getMemorySize(), coreId, coreSlots, false);
				}
			}
		}
