...
public interface PriorityTasksItf {
    ...
    void normalTask(@Parameter(type = Type.FILE, direction = Direction.OUT) String fileName);

    @Method(declaringClass = "priorityTasks.PriorityTasksImpl", priority = Constants.IS_PRIORITARY_TASK)
    void priorityTask(@Parameter(type = Type.FILE, direction = Direction.OUT) String fileName);
