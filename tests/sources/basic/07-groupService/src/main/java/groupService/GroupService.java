...
@WebService(serviceName = "GroupService",
			portName = "GroupServicePort",
			targetNamespace = "http://groupService")
public class GroupService {
	...
	public void setNumWorkers(@WebParam(name="numWorkers") int n) {
		if (n >= 0) {
			...
			for (int i = 0; i < numWorkers; ++i) workers[i] = new Person();
		}
	}
	
	@WebMethod
	public Person getWorker (@WebParam(name="id") int id) {
		if ((id >= 0) && (id < numWorkers)) return new Person(workers[id]);
		return null;
	}
