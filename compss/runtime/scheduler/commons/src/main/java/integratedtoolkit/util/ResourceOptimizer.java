...
public class ResourceOptimizer extends Thread {
	...
	// Loggers
	protected static final Logger RESOURCES_LOGGER = LogManager
			.getLogger(Loggers.RESOURCES);
	protected static final Logger RUNTIME_LOGGER = LogManager
			.getLogger(Loggers.RM_COMP);
	...
	public ResourceOptimizer(TaskScheduler ts) {
		if (DEBUG) {
			RUNTIME_LOGGER
					.debug("[Resource Optimizer] Initializing Resource Optimizer");
		}
				...
				if (CoreManager.getCoreCount() > 0) {
					do {
						...
						if (ResourceManager.useCloud()) {
							...
							int VMsBeingCreated = ResourceManager
									.getPendingCreationRequests().size();
		...
		if (potentialBlock) { // All tasks are blocked, and there are no
			...
			if (everythingBlockedRetryCount > 0) { // First time not taken into
				...
				if (everythingBlockedRetryCount < EVERYTHING_BLOCKED_MAX_RETRIES) {
					...
					int retriesLeft = EVERYTHING_BLOCKED_MAX_RETRIES
							- everythingBlockedRetryCount;
					ErrorManager
							.warn("No task could be scheduled to any of the available resources.\n"
	...
	public static int addBasicNodes() {
		...
		for (int coreId = 0; coreId < coreCount; coreId++) {
			if (unfulfilledConstraints[coreId].size() > 0) {
				unfulfilledConstraintsCores += unfulfilledConstraints[coreId]
						.size();
				break;
			}
		}
		 ...
		 * 
		 * e.g. architectures constraintsPerArquitecture Intel = |MR1|--|MR2|
		 * AMD = |MR3| [unassigned] = |MR4|--|MR5|
		 */
		Map<String, List<ConstraintsCore>> arch2Constraints = classifyArchitectures(unfulfilledConstraints);

		/*
		 * Tries to reduce the number of machines per architecture by entering
		 * constraints in another core's constraints
		 */
		reduceArchitecturesConstraints(arch2Constraints);

		/*
		 * Checks if there are enough Vm for a Unassigned Arquitecture If not it
		 * set each unassigned task into the architecture with the most similar
		 * task e.g. constraintsPerArquitecture Intel --> |MR1|--|MR2|--|MR5|
		 * AMD --> |MR3|--|MR4|
		 */
		...
		for (int coreId = 0; coreId < coreCount; coreId++) {
			while (!unfulfilledConstraints[coreId].isEmpty()) {
				...
				if (rcr != null) {
					...
					for (Entry<CloudInstanceTypeDescription, int[]> entry : rcr
							.getRequested().getTypeComposition().entrySet()) {
					...
					if (DEBUG) {
						...
						for (int i = 0; i < rcr
								.requestedSimultaneousTaskCount().length; i++) {
							for (int j = 0; j < rcr
									.requestedSimultaneousTaskCount()[i].length; ++j) {
								sb.append("\t")
										.append("IMPLEMENTATION_INFO = [")
										.append("\n");
								sb.append("\t\t").append("COREID = ").append(i)
										.append("\n");
								sb.append("\t\t").append("IMPLID = ").append(j)
										.append("\n");
								sb.append("\t\t")
										.append("SIM_TASKS = ")
										.append(rcr
												.requestedSimultaneousTaskCount()[i][j])
										.append("\n");
	...
	// Removes from the list all the Constraints fullfilled by existing
	// resources
	...
	private static List<ConstraintsCore>[] getUnfulfilledConstraints() {
		...
		for (int coreId = 0; coreId < coreCount; coreId++) {
			...
			if (maxSimTasks[coreId] == 0) {
				List<Implementation> impls = CoreManager
						.getCoreImplementations(coreId);
				for (Implementation impl : impls) {
					if (impl.getTaskType() == TaskType.METHOD) {
						MethodResourceDescription requirements = (MethodResourceDescription) impl
								.getRequirements();
						CloudMethodResourceDescription cd = new CloudMethodResourceDescription(
								requirements);
						ConstraintsCore cc = new ConstraintsCore(cd, coreId,
								unfulfilledConstraints[coreId]);
	 ...
	 */
	private static Map<String, List<ConstraintsCore>> classifyArchitectures(
			List<ConstraintsCore>[] constraints) {
		...
		for (int coreId = 0; coreId < CoreManager.getCoreCount(); coreId++) {
			if (constraints[coreId] != null) {
				for (ConstraintsCore cc : constraints[coreId]) {
					List<String> runnableArchitectures = cc.desc
							.getArchitectures();
	...
	private static void reduceArchitecturesConstraints(
			Map<String, List<ConstraintsCore>> arch2Ctrs) {
				...
				for (int j = i + 1; j < ctrs.length; j++) {
					...
					if (OS.compareTo(ctrs[j].desc.getOperatingSystemType()) == 0
							...
							|| ctrs[j].desc
									...
									.compareTo(
											CloudMethodResourceDescription.UNASSIGNED_STR) == 0) {
						mergedTo[j] = i;
						ctrs[i].join(ctrs[j]);
						arch.remove(ctrs[j]);
					}
	...
	private static void reassignUnassignedConstraints(
			Map<String, List<ConstraintsCore>> arch2Ctrs) {
		/*
		 * ATTENTION: Since this method is only evaluated from constraints,
		 * there is only 1 PROCESSOR and 1 ARCHITECTURE
		 */
		List<ConstraintsCore> unassignedList = arch2Ctrs
				.get(CloudMethodResourceDescription.UNASSIGNED_STR);
		...
		if (arch2Ctrs.size() == 2) {
			for (Entry<String, List<ConstraintsCore>> ctrs : arch2Ctrs
					.entrySet()) {
				if (ctrs.getKey().compareTo(
						CloudMethodResourceDescription.UNASSIGNED_STR) == 0) {
					continue;
				} else {
				...
				if (bestDifference < 0) {
					if (difference < 0) {
						if (difference > bestDifference) {
							List<String> avail_archs = option
									.getArchitectures();
	...
	public static int addExtraNodes(int alreadyCreated) {
		...
		/*
		 * Tries to reduce the number of machines by entering methodConstraints
		 * in another method's machine
		 */
		/*
		 * LinkedList<CloudWorkerDescription> requirements = new
		 * LinkedList<CloudWorkerDescription>(); for (int coreId = 0; coreId <
		 * CoreManager.coreCount; coreId++) { Implementation impl =
		 * CoreManager.getCoreImplementations(coreId)[0]; if (impl.getType() ==
		 * Type.METHOD) { WorkerDescription wd = (WorkerDescription)
		 * impl.getRequirements(); requirements.add(new
		 * CloudWorkerDescription(wd)); } } if (requirements.size() == 0) {
		 * return 0; } requirements = reduceConstraints(requirements);
		 * 
		 * int numTasks = requirements.size(); int[] vmCountPerContraint = new
		 * int[numTasks]; int[] coreCountPerConstraint = new int[numTasks];
		 * 
		 * for (int index = 0; index < numTasks; index++) {
		 * vmCountPerContraint[index] = 1; coreCountPerConstraint[index] =
		 * requirements.get(index).getSlots(); }
		 * 
		 * for (int i = 0; i < vmCount; i++) { float millor = 0.0f; int opcio =
		 * 0; for (int j = 0; j < requirements.size(); j++) { if (millor <
		 * ((float) coreCountPerConstraint[j] / (float) vmCountPerContraint[j]))
		 * { opcio = j; millor = ((float) coreCountPerConstraint[j] / (float)
		 ...
		 * rcr.getRequested().getType() + "\n" + "\tPROVIDER = " +
		 * rcr.getProvider() + "\n" +
		 * "\tREASON = Fulfill the initial Cloud instances constraint\n" + "]");
		 * if (DEBUG) { StringBuilder sb = new
		 * StringBuilder("EXPECTED_INSTANCE_SIM_TASKS = ["); int[][]
		 * simultaneousImpls = rcr.requestedSimultaneousTaskCount(); for (int
		 * core = 0; core < simultaneousImpls.length; ++core) { int
		 * simultaneousTasks = 0; for (int j = 0; j <
		 * simultaneousImpls[core].length; ++j) { if (simultaneousTasks <
		 * simultaneousImpls[core][j]) { simultaneousTasks =
		 * simultaneousImpls[core][j]; } }
		 * sb.append("\t").append("CORE = [").append("\n");
		 * sb.append("\t").append
		 * ("\t").append("COREID = ").append(core).append("\n");
		 * sb.append("\t").
		 * append("\t").append("SIM_TASKS = ").append(simultaneousTasks
		 * ).append("\n"); sb.append("\t").append("]").append("\n");
	...
	protected void applyPolicies(WorkloadState workload) {
		...
		for (int coreId = 0; coreId < coreCount; coreId++) {
			minCoreTime[coreId] = Math.min(workload.getCoreMinTime(coreId),
					creationTime);
			meanCoreTime[coreId] = Math.min(workload.getCoreMeanTime(coreId),
					creationTime);
			maxCoreTime[coreId] = Math.min(workload.getCoreMaxTime(coreId),
					creationTime);
			...
			if (minCoreTime[coreId] - meanRunningCoreTime < 0) {
				...
				minRemainingCoreTime[coreId] = Math.min(minCoreTime[coreId]
						- meanRunningCoreTime, creationTime);
			}
			if (meanCoreTime[coreId] - meanRunningCoreTime < 0) {
				meanRemainingCoreTime[coreId] = 0;
			} else {
				meanRemainingCoreTime[coreId] = Math.min(meanCoreTime[coreId]
						- meanRunningCoreTime, creationTime);
			}
			if (maxCoreTime[coreId] - meanRunningCoreTime < 0) {
				maxRemainingCoreTime[coreId] = 0;
			} else {
				maxRemainingCoreTime[coreId] = Math.min(maxCoreTime[coreId]
						- meanRunningCoreTime, creationTime);
			}
		...
		for (int i = 0; i < coreCount; i++) {
			...
			readyMinCoreTime[i] = runningMinCoreTime[i]
					+ (minCoreTime[i] * readyCounts[i]);
			pendingMinCoreTime[i] = readyMinCoreTime[i]
					+ (minCoreTime[i] * pendingCounts[i]);
			runningMeanCoreTime[i] = meanRemainingCoreTime[i]
					* runningCounts[i];
			readyMeanCoreTime[i] = runningMeanCoreTime[i]
					+ (meanCoreTime[i] * readyCounts[i]);
			pendingMeanCoreTime[i] = readyMeanCoreTime[i]
					+ (meanCoreTime[i] * pendingCounts[i]);
			runningMaxCoreTime[i] = maxRemainingCoreTime[i] * runningCounts[i];
			readyMaxCoreTime[i] = runningMaxCoreTime[i]
					+ (maxCoreTime[i] * readyCounts[i]);
			pendingMaxCoreTime[i] = readyMaxCoreTime[i]
					+ (maxCoreTime[i] * pendingCounts[i]);
			totalPendingTasks = totalPendingTasks + pendingCounts[i];
		}
		if (DEBUG) {
			RUNTIME_LOGGER
					.debug("[Resource Optimizer] Applying VM optimization policies (currentVMs: "
							+ currentCloudVMCount
							+ " maxVMs: "
							+ maxNumberOfVMs
							+ " minVMs: "
		...
		if (!cleanUp) {
			...
			List<Integer> requiredVMs = checkNeededMachines(noResourceCount,
					noResourceCounts, totalSlots);

			if (!requiredVMs.isEmpty()) {
				RUNTIME_LOGGER
						.debug("[Resource Optimizer] Required VMs. Mandatory Increase");
				float[] creationRecommendations = recommendCreations(coreCount,
						creationTime, readyMinCoreTime, readyMeanCoreTime,
						readyMaxCoreTime, totalSlots, realSlots);
				for (Integer coreId : requiredVMs) {
					// Ensure that we ask one slot for it
					creationRecommendations[coreId] = Math.max(
							creationRecommendations[coreId], 1);
				}
			...
			if (minNumberOfVMs != null && minNumberOfVMs > currentCloudVMCount) {
				RUNTIME_LOGGER.debug("[Resource Optimizer] Current VM ("
						+ currentCloudVMCount
						+ ") count smaller than minimum VMs (" + minNumberOfVMs
						+ "). Mandatory Increase");
				float[] creationRecommendations = orderCreations(coreCount,
						creationTime, readyMinCoreTime, readyMeanCoreTime,
						readyMaxCoreTime, totalSlots, realSlots);
			...
			if (maxNumberOfVMs != null && maxNumberOfVMs < currentCloudVMCount) {
				RUNTIME_LOGGER.debug("[Resource Optimizer] Current VM ("
						+ currentCloudVMCount
						+ ") count bigger than maximum VMs (" + maxNumberOfVMs
						+ "). Mandatory reduction");
				float[] destroyRecommendations = deleteRecommendations(
						coreCount, SLEEP_TIME, pendingMinCoreTime,
						pendingMeanCoreTime, pendingMaxCoreTime, totalSlots,
						realSlots);
		...
		if ((minNumberOfVMs == null || minNumberOfVMs < currentCloudVMCount)
				&& totalPendingTasks == 0 && workload.getReadyCount() == 0) {
			RUNTIME_LOGGER.debug("[Resource Optimizer] Current VM ("
					+ currentCloudVMCount + ") count bigger than minimum VMs ("
					+ minNumberOfVMs + ")");
			float[] destroyRecommendations = deleteRecommendations(coreCount,
					SLEEP_TIME, pendingMinCoreTime, pendingMeanCoreTime,
	...
	private void mandatoryIncrease(float[] creationRecommendations,
			List<Integer> requiredVMs) {
		...
		for (int coreId = 0; coreId < creationRecommendations.length; coreId++) {
			...
			for (Implementation impl : impls) {
				...
				MethodResourceDescription constraints = ((MethodImplementation) impl)
						.getRequirements();
				ValueResourceDescription v = new ValueResourceDescription(
						constraints, creationRecommendations[coreId], false);
				pq.add(v);
			}
		}
	...
	private boolean optionalReduction(float[] destroyRecommendations) {
		List<CloudMethodWorker> nonCritical = trimReductionOptions(
				ResourceManager.getNonCriticalDynamicResources(),
				destroyRecommendations);
		if (DEBUG) {
			RUNTIME_LOGGER
					.debug("[Resource Optimizer] Searching for best destruction");
		}
		Object[] nonCriticalSolution = getBestDestruction(nonCritical,
				destroyRecommendations);
		...
		if (DEBUG) {
			RUNTIME_LOGGER
					.debug("[Resource Optimizer] Best resource to remove is "
							+ res.getName() + "and record is [" + record[0]
							+ "," + record[1] + "," + record[2]);
		}
		if (record[1] > 0 && res.getUsedCPUTaskCount() > 0) {
			RUNTIME_LOGGER
					.debug("[Resource Optimizer] Optional destroy recommendation not applied");
			return false;
		} else {
			RUNTIME_LOGGER
					.debug("[Resource Optimizer] Optional destroy recommendation applied");
			CloudMethodResourceDescription finalDescription = new CloudMethodResourceDescription(
					rd, res.getDescription().getImage());
	...
	private void mandatoryReduction(float[] destroyRecommendations) {
		List<CloudMethodWorker> critical = trimReductionOptions(
				ResourceManager.getCriticalDynamicResources(),
				destroyRecommendations);
		...
		List<CloudMethodWorker> nonCritical = trimReductionOptions(
				ResourceManager.getNonCriticalDynamicResources(),
				destroyRecommendations);
		Object[] criticalSolution = getBestDestruction(critical,
				destroyRecommendations);
		Object[] nonCriticalSolution = getBestDestruction(nonCritical,
				destroyRecommendations);
	...
	private List<CloudMethodWorker> trimReductionOptions(
			Collection<CloudMethodWorker> options, float[] recommendations) {
		if (DEBUG) {
			RUNTIME_LOGGER
					.debug("[Resource Optimizer] * Trimming reduction options");
		}
		...
		while (it.hasNext()) {
			...
			if (DEBUG) {
				RUNTIME_LOGGER.debug("\t Evaluating " + resource.getName()
						+ ". Default reduction is " + add);
			}
			...
			for (int coreId : executableCores) {
				...
				if (!aggressive && recommendations[coreId] < 1) {
					if (DEBUG) {
						RUNTIME_LOGGER
								.debug("\t\tVM not removed because of not agressive and recomendations < 1 ("
				...
				if (aggressive && recommendations[coreId] > 0) {
					if (DEBUG) {
						RUNTIME_LOGGER
								.debug("\t\tVM removed because of agressive and recomendations > 0 ("
										+ recommendations[coreId] + ")");
					}
					add = true;
					break;
			...
			if (add) {
				if (DEBUG) {
					RUNTIME_LOGGER
							.debug("\t\tVM added to candidate to remove.");
				}
	...
	private static ResourceCreationRequest requestOneCreation(
			PriorityQueue<ValueResourceDescription> pq, boolean include) {
		ValueResourceDescription v;
		while ((v = pq.poll()) != null) {
			ResourceCreationRequest rcr = askForResources(v.value < 1 ? 1
					: (int) v.value, v.constraints, include);
			if (rcr != null) {
				StringBuilder compositionString = new StringBuilder();
				for (java.util.Map.Entry<CloudInstanceTypeDescription, int[]> entry : rcr
						.getRequested().getTypeComposition().entrySet()) {
					compositionString.append(" \t\tTYPE = [\n")
							.append("\t\t\tNAME = ")
							.append(entry.getKey().getName())
				...
				if (DEBUG) {
					...
					for (int i = 0; i < CoreManager.getCoreCount(); i++) {
						for (int j = 0; j < rcr
								.requestedSimultaneousTaskCount()[i].length; ++j) {
							sb.append("\t").append("IMPLEMENTATION_INFO = [")
									.append("\n");
							sb.append("\t\t").append("COREID = ").append(i)
									.append("\n");
							sb.append("\t\t").append("IMPLID = ").append(j)
	 ...
	 * ***************************************
	 */
	private List<Integer> checkNeededMachines(int noResourceCount,
			int[] noResourceCountPerCore, int[] slotCountPerCore) {
	...
	private float[] recommendCreations(int coreCount, long creationTime,
			long[] aggregatedMinCoreTime, long[] aggregatedMeanCoreTime,
		...
		for (int coreId = 0; coreId < coreCount; coreId++) {
			...
			long remainingLoad = aggregatedMeanCoreTime[coreId]
					- embraceableLoad;
			if (DEBUG) {
				RUNTIME_LOGGER
						.debug("[Resource Optimizer] * Calculating increase recomendations");
				RUNTIME_LOGGER.debug("\tRemaining load = "
						+ aggregatedMeanCoreTime[coreId] + "-( "
						+ totalSlots[coreId] + " * " + creationTime + " ) = "
						+ remainingLoad);
			}
			if (remainingLoad > 0) {
				creations[coreId] = (int) (remainingLoad / creationTime);
				if (DEBUG) {
					RUNTIME_LOGGER.debug("\tRecomended slots = "
							+ creations[coreId] + " ( " + remainingLoad + " / "
							+ creationTime + " )");
				}
			} else {
				creations[coreId] = 0;
	...
	private float[] orderCreations(int coreCount, long creationTime,
			long[] aggregatedMinCoreTime, long[] aggregatedMeanCoreTime,
			long[] aggregatedMaxCoreTime, int[] totalSlots, int[] realSlots) {
	...
	private float[] deleteRecommendations(int coreCount, long limitTime,
			long[] aggregatedMinCoreTime, long[] aggregatedMeanCoreTime,
			long[] aggregatedMaxCoreTime, int[] totalSlots, int[] realSlots) {

		if (DEBUG) {
			RUNTIME_LOGGER
					.debug("[Resource Optimizer] * Delete Recomendations calculations:\n\tcoreCount: "
							+ coreCount + " limitTime: " + limitTime);
		}
		...
		for (int coreId = 0; coreId < coreCount; coreId++) {
			...
			if (embraceableLoad == 0l) {
				...
				if (DEBUG) {
					RUNTIME_LOGGER.debug("\tembraceableLoad [ " + coreId
							+ "] = " + limitTime + " * " + realSlots[coreId]
							+ " = " + embraceableLoad);
					RUNTIME_LOGGER
							.debug("\tdestructions [ " + coreId + "] = 0");
				}
			} else if (aggregatedMinCoreTime[coreId] > 0) {
				double unusedTime = (((double) embraceableLoad) / (double) 2)
						- (double) aggregatedMeanCoreTime[coreId];
				destructions[coreId] = (float) (unusedTime / (double) limitTime);
				if (DEBUG) {
					RUNTIME_LOGGER.debug("\tembraceableLoad [ " + coreId
							+ "] = " + limitTime + " * " + realSlots[coreId]
							+ " = " + embraceableLoad);
					RUNTIME_LOGGER.debug("\tunused [ " + coreId + "] =  ( "
							+ embraceableLoad + " / 2) - "
							+ aggregatedMeanCoreTime[coreId] + " = "
							+ unusedTime);
					RUNTIME_LOGGER.debug("\tdestructions [ " + coreId + "] = "
							+ destructions[coreId]);
				}
			} else {
	...
	private static class ConstraintsCore {
		...
		@SuppressWarnings("unchecked")
		public ConstraintsCore(CloudMethodResourceDescription desc, int core,
				List<ConstraintsCore> coreList) {
			this.desc = desc;
	...
	private static class ValueResourceDescription implements
			Comparable<ValueResourceDescription> {

		private final MethodResourceDescription constraints;
		private final float value;
		private final boolean prioritary;

		public ValueResourceDescription(MethodResourceDescription constraints,
				float value, boolean prioritary) {
	...
	public ResourceCreationRequest askForResources(CloudProvider cp,
			String instanceName, String imageName) {
		CloudMethodResourceDescription constraints = cp.getResourceDescription(
	 ...
	 * @param contained
	 *            {@literal true} if we want the request to ask for a resource
	 *            contained in the description; else, the result contains the
	 *            passed in description.
	 * @return Description of the ResourceRequest sent to the CloudProvider.
	 *         {@literal Null} if any of the Cloud Providers can offer a
	 *         resource like the requested one.
	 */
	public static ResourceCreationRequest askForResources(
	 ...
	 * @return
	 */
	public static ResourceCreationRequest askForResources(Integer amount,
			MethodResourceDescription requirements, boolean contained) {
		...
		for (CloudProvider cp : ResourceManager.getAvailableCloudProviders()) {
			CloudMethodResourceDescription rc = getBestIncreaseOnProvider(cp,
					amount, requirements, contained);
			if (rc != null && rc.getValue() < bestValue) {
				...
				RUNTIME_LOGGER.warn(WARN_NO_POSIBLE_INCREASE + " ("
						+ cp.getName() + ")");
			}
		}
	...
	public static CloudMethodResourceDescription getBestIncreaseOnProvider(
			CloudProvider cp, Integer amount,
			MethodResourceDescription requirements, boolean contained) {
		RUNTIME_LOGGER
				.debug("[Resource Optimizer] Getting best increase in provider "
						+ cp.getName());
		...
		List<CloudInstanceTypeDescription> instances = cp
				.getCompatibleTypes(requirements);
		RUNTIME_LOGGER.debug("[Resource Optimizer] There are "
				+ instances.size() + " instances compatible.");
		...
		if (type != null) {
			...
			List<CloudImageDescription> images = cp
					.getCompatibleImages(requirements);
			RUNTIME_LOGGER.debug("[Resource Optimizer] There are "
					+ images.size() + " images compatible.");
			if (!images.isEmpty()) {
	...
	private static CloudInstanceTypeDescription selectContainingInstance(
			List<CloudInstanceTypeDescription> instances,
		...
		for (CloudInstanceTypeDescription type : instances) {
			...
			RUNTIME_LOGGER.debug("[Resource Optimizer] Can host: slots = "
					+ slots + " amount = " + amount + " distance = " + distance
	...
	private static CloudInstanceTypeDescription selectContainedInstance(
			List<CloudInstanceTypeDescription> instances,
			MethodResourceDescription constraints, int amount) {
	 ...
	 *
	 *         1->(CloudTypeInstanceDescription) Type to be destroyed to be
	 *         destroyed.
	 *
	 *         2-> (int[]) record of the #CE with removed slots and that they
	 *         shouldn't be modified, #slots that will be destroyed and they
	 *         weren't recommended, #slots that will be removed and they were
	 *         asked to be.
	 *
	 */
	private Object[] getBestDestruction(
			Collection<CloudMethodWorker> resourceSet,
		...
		for (CloudMethodWorker res : resourceSet) {
			...
			if (cp == null) { // it's not a cloud machine
				if (DEBUG) {
					RUNTIME_LOGGER.debug("Resource " + res.getName()
							+ " is not cloud. Skipping...");
				}
				continue;
			}

			Map<CloudInstanceTypeDescription, float[]> typeToPoints = getPossibleReductions(
					res, destroyRecommendations);

			for (Map.Entry<CloudInstanceTypeDescription, float[]> destruction : typeToPoints
					.entrySet()) {
				CloudInstanceTypeDescription type = destruction.getKey();
				float[] values = destruction.getValue();
				if (DEBUG) {
					RUNTIME_LOGGER.debug("Type: " + type.getName()
							+ " value 0: " + values[0] + " (" + bestRecord[0]
							+ ") " + " value 1: " + values[1] + " ("
							+ bestRecord[1] + ") " + " value 2: " + values[2]
							+ " (" + bestRecord[2] + ")");
				}
	...
	private Map<CloudInstanceTypeDescription, float[]> getPossibleReductions(
			CloudMethodWorker res, float[] recommendedSlots) {
		Map<CloudInstanceTypeDescription, float[]> reductions = new HashMap<>();
		List<CloudInstanceTypeDescription> types = res.getDescription()
				.getPossibleReductions();
		...
		for (CloudInstanceTypeDescription type : types) {
			...
			for (int coreId = 0; coreId < CoreManager.getCoreCount(); coreId++) {
				if (res.canRun(coreId)) {
					if (recommendedSlots[coreId] < 1
							&& reducedSlots[coreId] > 0) {
						values[0]++; // Adding a desired CE whose slots will be
						// destroyed
						values[1] += reducedSlots[coreId]; // all reduced slots
						// weren't requested
					} else {
						float dif = (float) reducedSlots[coreId]
								- recommendedSlots[coreId];
						if (dif < 0) {
