...
public class GATJob extends integratedtoolkit.types.job.Job<GATWorkerNode> implements MetricListener {
	...
	public void processMetricEvent(MetricEvent value) {
		...
		/*
		 * Check if either the job has finished or there has been a submission
		...
		if (newJobState == JobState.STOPPED) {
			...
			/*
			 * We must check whether the chosen adaptor is globus In that case,
			 * since globus doesn't provide the exit status of a job, we must
			 * examine the standard error file
			 */
			try {
				if (usingGlobus) {
	...
	private JobDescription prepareJob() throws Exception {
		...
		// method_class method_name has_target num_params par_type_1 par_1 ...
		// par_type_n par_n
		...
		try {
			...
			StringBuilder sb = new StringBuilder("Null argument parameter of job " + this.jobId + "(" + methodName + "@"
		 ...
		 * sd.addAttribute(SoftwareDescription.SANDBOX_POSTSTAGE_STDOUT,
		 * "false");
		 * sd.addAttribute(SoftwareDescription.SANDBOX_POSTSTAGE_STDERR,
		 * "false");
		 */
