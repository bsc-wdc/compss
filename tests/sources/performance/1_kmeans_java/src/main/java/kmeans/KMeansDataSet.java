...
public class KMeansDataSet {
     ...
     */
    /*
     * public static boolean writePointsToFile(String fileName, KMeansDataSet data) { int numPoints = data.numPoints; if
     * (numPoints == 0) return false; int numDimensions = data.numPoints; try { File outputFile = new File(fileName);
     * DataOutputStream out = new DataOutputStream(new FileOutputStream(outputFile)); out.writeInt(cookie);
     * out.writeInt(version); out.writeInt(numPoints); out.writeInt(numDimensions); for (int i=0;
     * i<numPoints*numDimensions; i++) { out.writeFloat(data.points[i]); }
     * 
     * } catch (FileNotFoundException e) { System.out.println("Unable to open file for writing "+fileName); return
     * false; } catch (IOException e) { e.printStackTrace(); return false; }
     * 
     * return true; }
     */
    /**
     ...
     * int numPoints = 0; float[] points = null;
     * 
     * try { DataInputStream data = new DataInputStream(new FileInputStream(new File(fileName))); int fc =
     * data.readInt(); if (fc != cookie) { System.err.printf("Invalid cookie.  Found %d but expected %d\n", fc, cookie);
     * } int fv = data.readInt(); if (fv != version) { System.err.printf("Invalid version.  Found %d but expected %d\n",
     * fc, cookie); } numPoints = data.readInt(); numDimensions = data.readInt(); points = new
     * float[numPoints*numDimensions]; System.out.printf("Reading %d %d-dimensional points from %s\n", numPoints,
     * numDimensions, fileName); for (i=0; i<numPoints; i++) { for (j=0; j<numDimensions; j++) { points[i*numDimensions+
     * j] = data.readFloat(); } } } catch (FileNotFoundException e) {
     * System.err.println("Unable to open file "+fileName); System.exit(-1); } catch (IOException e) {
