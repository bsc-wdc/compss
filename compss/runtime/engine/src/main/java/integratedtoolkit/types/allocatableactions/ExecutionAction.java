...
public abstract class ExecutionAction<P extends Profile, T extends WorkerResourceDescription> extends AllocatableAction<P, T> {
    ...
    public void failedTransfers(int failedtransfers) {
        ...
        if (transferErrors < TRANSFER_CHANCES) {
            jobLogger.debug("Resubmitting input files for task " + task.getId() + " to host "
                    + selectedMainResource.getName() + " since " + failedtransfers + " transfers failed.");

            transferInputData();
        } else {
            ErrorManager.warn("Transfers for running task " + task.getId() + " on worker " + selectedMainResource.getName() + " have failed.");
            this.notifyError();
        }
    }
    ...
    public void failedJob(Job<?> job, Job.JobListener.JobEndStatus endStatus) {
        ...
        if ((transferErrors + executionErrors) < SUBMISSION_CHANCES) {
            jobLogger.error("Job " + job.getJobId() + " for running task " + task.getId() + " on worker " + selectedMainResource.getName() + " has failed; resubmitting task to the same worker.");
            ErrorManager.warn("Job " + job.getJobId() + " for running task " + task.getId() + " on worker " + selectedMainResource.getName() + " has failed; resubmitting task to the same worker.");
    ...
    protected void doError() throws FailedActionException {
        if (this.executingResources.size() >= SCHEDULING_CHANCES) {
            ...
            ErrorManager.warn("Task " + task.getId() + " execution on worker " + selectedMainResource.getName() + " has failed; rescheduling task execution. (changing worker)");
            logger.error("Task " + task.getId() + " execution on worker " + selectedMainResource.getName() + " has failed; rescheduling task execution. (changing worker)");
        }
    }
