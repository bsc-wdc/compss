            ...
            for i, item in enumerate(common_paths):  # All files must have a match
                if url_parts.path.startswith(item):
                    if len(common_paths) == 1:
                        ...
                        cp_path = Path(item)  # Looking for the name of the previous folder
                        crate_path = (
                            "dataset/"
                            # + "folder_"
                            # + str(i)
                            + cp_path.parts[-1]  # Base name of the identified common path. Now it does not avoid collisions if the user defines the same folder name in two different locations
...
def get_common_paths(url_list: list) -> list:
    ...
    # The list comes ordered, so all dir:// references will come first, and we need to skip them
    i = 0
    file_found = False
    for item in url_list:
        ...
        if url_parts.scheme == "dir":
            i += 1
            # print(f"PROVENANCE DEBUG | SKIPPING DIRECTORY. i is: {i}")
            continue
        else:
            file_found = True
            break
