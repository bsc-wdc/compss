...
def updateTasksCodeFile(f, filePath):   # TODO - ALSO GET CLASSES DEFINED BY USERS
    """
    ...
    if not os.path.exists(filePath):
        createTasksCodeFile(filePath)    # TODO ADD CLASSES SEPARATOR
    ...
    # classesCode = getClasses()        ######## {'name': str(line\nline\n...)} # TODO GET CLASSES DEFINED BY THE USER IN THE INTERACTIVE SESSION
    ...
    # newClasses = updateClasses(classesCode, oldCode['classes'])   # TODO COMPARE THE EXISTING CLASSES WITH THE NEW ONES
    newTasks = updateTasks(taskCode, oldCode['tasks'])

    # Update the file where the code is stored.
    updateCodeFile(newImports, newGlobals, newFunctions, newTasks, filePath)  # TODO ADD CLASSES TO RESULT FILE
...
def getFunctions():
        ...
        for l in lines:
            ...
            if isFunction:
                if l.startswith("def"):
                    ...
                    funcName = header[1].split("(")[0]  # the name may be followed by the parameters parenthesis
    ...
    for k,v in functions.iteritems():
        functions[k] = ''.join(v)      # Collapse all lines into a single one
    return functions
...
def reduce(linesList):
    """
    Removes the blank lines from a list of strings keeping some of them.
    * getOldCode auxiliar method - Clean tasks code (keeps the '\n' between tasks).
    :param linesList: List of strings
    :return: The list without some '\n' strings.
    """
    result = []
    jump = False
    for l in linesList:
...
def getOldCode(filePath):
    ...
    for line in contents:
        ...
        if line == separators['globals_separator'] + '\n':
            ...
            if not foundGlobSeparator and not foundFuncSeparator and not foundTaskSeparator:
                fileImports.append(line)
            elif foundGlobSeparator and not foundFuncSeparator and not foundTaskSeparator:
                fileGlobals.append(line)
            elif foundGlobSeparator and foundFuncSeparator and not foundTaskSeparator:
    ...
    fileTasks = clean(fileTasks) # reduce(fileTasks)

    # Process functions
    functions = {}
    collapsed = ''.join(fileFunctions)      # Collapse all lines into a single one
    ...
    for f in funcs:
        funcCode = f
        funcName = f.replace('(', ' (').split(' ')[1]
        functions[funcName] = funcCode

    # Process tasks
    tasks = {}
    collapsed = ''.join(fileTasks)  # Collapse all lines into a single one
    ...
    # Add functions to dictionary by function name:
    for t in tsks:
        taskCode = t   # Example: '@task(returns=int)\ndef mytask(v):\n    return v+1\n'
        taskHeader = t.split('\ndef')[1]
        taskName = taskHeader.replace('(', ' (').split(' ')[1]
        tasks[taskName] = taskCode

    return {'imports': fileImports, 'globals': fileGlobals, 'functions':functions, 'tasks': tasks}
    ...
    for i in newGlobals:
        ...
        for j in oldGlobals:
            if i == j:
                already = True
        if not already:
            notInGlobals.append(i)
    # Merge the minimum globals
    imports = oldGlobals + notInGlobals
    return imports
...
def updateCodeFile(newImports, newGlobals, newFunctions, newTasks, filePath):
    """
