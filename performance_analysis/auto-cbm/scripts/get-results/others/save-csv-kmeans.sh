...
# This script saves csvs for different applications like Kmeans, KMeans, etc.
AUTO_CBM_DIR=../../..
RES_DIR=${AUTO_CBM_DIR}/results
KMEANS_RES_DIR=${RES_DIR}/kmeans-csvs
CSV=${KMEANS_RES_DIR}/kmeans.csv

mkdir -p "${KMEANS_RES_DIR}"
touch ${CSV}
echo "BSIZE,MSIZE,Matrix_Size,Elapsed_Seconds" >> ${CSV}

for FILE_OUT in ${RES_DIR}/kmeans-data/*
do
