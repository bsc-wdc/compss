...
import es.bsc.compss.api.impl.DoNothingWorkflowListener;
...
public class Application implements ApplicationTaskMonitor, DataOwner {
    ...
    private static final WorkflowListener DEFAULT_LISTENER = new DoNothingWorkflowListener();
    ...
    protected Application(String parallelismSource, WorkflowListener listener) {
        this(getUniqueId(), parallelismSource, listener);
    }

    protected Application(Long appId, String parallelismSource, WorkflowListener listener) {
        this.id = appId;
     ...
     */
    public void stalled() {
        this.listener.onSynchronization();
    }
    ...
    public void readyToContinue(Semaphore sem) {
        this.listener.onReadyToContinue(sem);
    }
    ...
    /**
     * Returns a monitor for one of the tasks generated by the workflow.
     * 
     * @return
     */
    public TaskMonitor getTaskMonitor() {
        TaskMonitor m = this.listener.getTaskMonitor();
        if (m == null) {
            m = DEFAULT_LISTENER.getTaskMonitor();
        }
        return m;
    }
