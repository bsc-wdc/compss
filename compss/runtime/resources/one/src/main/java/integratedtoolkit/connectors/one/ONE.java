...
public class ONE extends AbstractSSHConnector {
    ...
    private Template classifyMachine(int cpu, float memory, float disk) {
        ...
        for (Template temp : pool) {
            ...
            logger.debug("Comparing with template " + temp.getName() + ": CPU=" + tmpCPU + ", Mem=" + tmpMem + ", Disk=" + tmpDisk);

            if (cpu <= tmpCPU && tmpCPU <= minCPU && memory <= tmpMem && tmpMem <= minMem && disk <= tmpDisk && tmpDisk <= minDisk) {
