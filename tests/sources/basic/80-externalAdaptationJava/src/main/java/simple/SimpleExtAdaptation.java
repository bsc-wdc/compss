...
public class SimpleExtAdaptation {
	...
	public static void main(String[] args) {
		...
		if (args.length != 5) {
			...
			System.out
					.println("    Usage simple <initVal> <increment> <minVM> <maxVM> <creationTime>");
			System.exit(-1);
		}
		...
		try {
			...
			System.out
					.println("[LOG] Initial counter value is " + initialValue);
			System.out.println("[LOG] Creating VM time is " + creationTime);

			// Check initial values
			int currentRes = ResourceManager.getAllWorkers().size();
			System.out.println("[LOG] Initial number of resources is "
					+ currentRes);
			if (currentRes != 0) {
				System.out.println("FAIL: Initial Resources incorrect "
						+ currentRes);
				System.exit(-1);
			} else {
			...
			System.out
					.println("[LOG] Creating minimal number of VM's. Waiting...");
			...
			if (currentRes != minVM) {
				System.out.println("FAIL: Initial VMs incorrect " + currentRes
						+ " (" + minVM + ")");
				System.exit(-1);
			} else {
			...
			System.out
					.println("[LOG] Creating manually an extra VMs. Waiting...");
			try {
			...
			currentRes = ResourceManager.getAllWorkers().size();
			if (currentRes != maxVM) {
				System.out.println("FAIL: Max VMs incorrect " + currentRes
						+ " (" + maxVM + ")");
				System.exit(-1);
			} else {
			...
			for (int i = 0; i < increment; i++) {
				...
				System.out.println("[LOG] Final counter" + i + " value is "
						+ finalValue + " (expected: " + expected + ")");
				fis.close();
			...
			System.out.println("Waiting " + sleepTime
					+ " ms for the elastic VMs to be removed");
			...
			currentRes = ResourceManager.getAllWorkers().size();
			if (currentRes != minVM) {
				System.out.println("VM not removed " + currentRes + " ("
						+ minVM + ")");
				System.exit(-1);
			} else {
