...
def write_data_local_db(data, application_name):
    try:
        with open(f'/home/ngiacomi/Documents/applications-test/ProvenanceStats/{application_name}.json', 'r') as json_object:
    ...
    with open(f'/home/ngiacomi/Documents/applications-test/ProvenanceStats/{application_name}.json', 'w') as json_object:
        json.dump(content, json_object, indent=4)

def store_data(compss_path: str, stats_path: Path):
    forbidden_types = ['Dataset', 'CreativeWork', '[', 'File', 'WebSite', 'CreativeWork']
    execution_stats = ['maxTime', 'executions', 'avgTime', 'minTime', 'executionTime']
    profiling_stats = ['cpuMax', 'cpuAvg', 'memMax', 'memAvg', 'memMin', 'memAvg', 'byteSent', 'byteRecv']
    ...
    for stat in profiling_stats:
        if 'avg' in stat.lower():
            ...
            min_value = float('inf')
            for n in nodes:
                min_value = final_dict[n][stat] if float(final_dict[n][stat]) < float(min_value) else min_value
            final_dict['overall'][stat] = min_value
        elif 'max' in stat.lower():
            max_value = float('-inf')
            for n in nodes:
                max_value = final_dict[n][stat] if float(final_dict[n][stat]) > float(max_value) else max_value
