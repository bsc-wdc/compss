...
public class CleanerExecutor {
	 ...
	 *
	 * @param cleanScripts
	 *            list of locations where to find all the cleaning scripts that
	 *            must be executed
	 * @param cleanParams
	...
	public void executeScript(List<URI> cleanScripts, List<String> cleanParams) {
		...
		for (int i = 0; i < cleanScripts.size(); i++) {
			...
			try {
				if (!node.isUserNeeded() && script.getUserInfo() != null) { // Remove
																			// user
																			// from
																			// the
																			// URI
				...
				if (debug) {
					try {
						org.gridlab.gat.io.File outFile = GAT.createFile(node.getContext(),
								"any:///" + System.getProperty(ITConstants.IT_APP_LOG_DIR) + "/cleaner.out");
						sd.setStdout(outFile);
						org.gridlab.gat.io.File errFile = GAT.createFile(node.getContext(),
								"any:///" + System.getProperty(ITConstants.IT_APP_LOG_DIR) + "/cleaner.err");
