    ...
    with open(dp_log, "r", encoding="UTF-8") as dp_file:
        ...
        for line in dp_file:
            ...
            if len(line_record) == 2:
                if (
                    line_record[1] == "IN" or line_record[1] == "IN_DELETE"
                ):  # Can we have an IN_DELETE that was not previously an OUTPUT?
                    if (
                        line_record[0] not in outputs
                    ):  # A true INPUT, not an intermediate file
                        inputs.add(line_record[0])
                        ins_of_current_task.append(line_record[0])
                ...
                elif line_record[1] == "OUT":
                    outputs.add(line_record[0])
