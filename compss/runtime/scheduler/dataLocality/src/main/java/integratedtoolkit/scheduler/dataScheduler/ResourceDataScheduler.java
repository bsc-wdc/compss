...
public class ResourceDataScheduler<P extends Profile, T extends WorkerResourceDescription> extends ReadyResourceScheduler<P, T> {
     ...
     * @param w
     */
    public ResourceDataScheduler(Worker<T> w) {
        super(w);
    }

    @Override
    public Score generateResourceScore(AllocatableAction<P, T> action, TaskDescription params, Score actionScore) {
        double resourceScore = DataScore.calculateScore(params, myWorker);
        return new DataScore(actionScore.getActionScore(), 0, resourceScore, 0);
    ...
    public Score generateImplementationScore(AllocatableAction<P, T> action, TaskDescription params, Implementation<T> impl,
        ...
        if (w.canRunNow(impl.getRequirements())) {
            ...
            return new DataScore(resourceScore.getActionScore(), 3, resourceScore.getResourceScore(),
