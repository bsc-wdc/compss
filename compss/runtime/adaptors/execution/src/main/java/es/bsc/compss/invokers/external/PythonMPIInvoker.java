...
public class PythonMPIInvoker extends ExternalInvoker {
    ...
    private Object runPythonMPIInvocation() throws InvokeExecutionException {
    	...
    	int NUM_BASE_PYTHON_MPI_ARGS = 7;
        ...
        // Prepare command
        String[] cmd = new String[NUM_BASE_PYTHON_MPI_ARGS + binaryParams.size()];
        ...
        String PYCOMPSS_RELATIVE_PATH = File.separator + "Bindings" + File.separator + "python";
        String pythonVersion = ((PythonParams) context.getLanguageParams(COMPSsConstants.Lang.PYTHON)).getPythonVersion();
        String pyCOMPSsHome = installDir + PYCOMPSS_RELATIVE_PATH + File.separator + pythonVersion;
        
        cmd[5] =  pyCOMPSsHome + File.separator + "pycompss" + File.separator+"worker" + File.separator + "mpi_executor.py";
        ...
        for (int i = 0; i < binaryParams.size(); ++i) {
        	cmd[NUM_BASE_PYTHON_MPI_ARGS + i] = binaryParams.get(i);
        }
