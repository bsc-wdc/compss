...
integer(kind=2),dimension(1:kdatamax):: &
 itemp1 &                    !
,itemp2                      !
...
character*256 input_seamask, input_soiltemp_path, output_deeptemperature

if (command_argument_count().gt.0) then
  call get_command_argument(1,input_seamask,length,stat)
  call get_command_argument(2,input_soiltemp_path,length,stat)
  call get_command_argument(3,output_deeptemperature,length,stat)
end if
...
!--merge the two data files cropping the boundary rows-----------------
      do jdata=1,jdatamax
        ...
        do idata=1,idatamax
          data(idata,jdata)=itemp2(idatabase+nbndry+idata)
        enddo
      enddo
      do jdata=1,jdatamax
        idatabase=(nbndry+jdata-1)*(idatamax+2*nbndry)
        do idata=1,idatamax
          data(idatamax+idata,jdata)=itemp1(idatabase+nbndry+idata)
        enddo
      enddo
!--interpolation loop--------------------------------------------------
      do j=1,jmi
...
!print*,'pp,qq,snoalb',pp,qq,deeptemperature(i,j)
!stop
        enddo
!stop
      enddo
!----------------------------------------------------------------------
      do j=jmi,1,-10
        write(*,1100) (deeptemperature(i,j),i=1,imi,40)
      enddo
!----------------------------------------------------------------------
      outfile = output_deeptemperature
      open(unit=1,file=outfile,status='unknown' &
          ,form='unformatted')
      write(1) deeptemperature
      close(1)
      print*,'Deep temperature written to ../output/deeptemperature'
      print*,'Enjoy your deep soil temperature!'
!----------------------------------------------------------------------
endprogram deeptemp
!----------------------------------------------------------------------
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
