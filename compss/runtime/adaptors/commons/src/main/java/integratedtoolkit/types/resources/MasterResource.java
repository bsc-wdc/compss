	...
	public MasterResource() {
		...
		if (mustCreateExecutionSandbox) {
			...
			if (System.getProperty(ITConstants.IT_SERVICE_NAME) != null) {
				/*
				 * SERVICE - Gets appName - Overloads the service folder for
				 * different executions - MAX_OVERLOAD raises warning - Changes
				 * working directory to serviceName !!!!
				 */
				...
				/*
				 * REGULAR APPLICATION - Gets appName - Overloads the app folder
				 * for different executions - MAX_OVERLOAD raises warning -
		...
		/*
		 * Create a tmp directory where to store: - Files whose first opened
		 * stream is an input one - Object files
		 */
		...
		/*
		 * Create a jobs dir where to store: - Jobs output files - Jobs error
		 * files
		 */
		...
		/*
		 * Create a workers dir where to store: - Worker out files - Worker
		 * error files
		 */
