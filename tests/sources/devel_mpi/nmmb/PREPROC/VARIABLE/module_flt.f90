...
!
            if(j.lt.10) &
            print*,'hfilt ',j,k,nsmud,hfilt(k,j),hfilt(k+1,j)
!
          enddo
          x=icycle*dlm*0.25
          xl=min(x/rcph*cfilt,pih)
...
!-----------------------------------------------------------------------
 ...
 200  continue
      ...
      print*,j,ks,nsmud,kvfilt(j)
      enddo
      kvfilt(jmi)=kvfilt(jmi-1)
          do k=1,icycle
      vfilt(k,jmi)=vfilt(k,jmi-1)
          enddo
...
integer(kind=kint),dimension(1:jmi),intent(in):: &
 khfilt

real(kind=kfpt),dimension(1:imi,1:jmi),intent(in):: &
 hfilt

real(kind=kfpt),dimension(1:imi,1:jmi),intent(inout):: &
 h2

real(kind=kfpt),dimension(1:imi-1):: &
 buff
...
real(kind=kfpt),dimension(1:imi,1:jmi,1:km),intent(inout):: &
 hn
