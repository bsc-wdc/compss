...
def plot_bytes(time_list, first_df, first_df_name, second_df, second_df_name, num_entries, title):
    plt.style.use('ggplot')
    fig, ax = plt.subplots(figsize=(12, 8))
    ...
    ax.plot(time_list, second_df, c='b', marker='.', linestyle='-', label=second_df_name)
...
def build_plot_nodes(time_list, df_list, num_entries, colors, metric_name):
    ...
    for (label, values), i in zip(df_list.items(), range(len(df_list))):
        index = i % len(colors)
        plt.plot(values[:num_entries], label=label, color=colors[index], marker='.', linestyle='-')
...
def plot_results(folder_pathname):
    ...
    for csv_resources in os.listdir(folder_pathname):
        ...
        plot_bytes(time_list=timestamps, first_df=byte_sent, first_df_name="Bytes sent", second_df=byte_recv, second_df_name="Bytes received", num_entries=df_lenght, title="Network usage")
        ...
        plot_bytes(time_list=timestamps, first_df=byte_write_disk, first_df_name="Bytes written", second_df=byte_read_disk, second_df_name="Bytes read", num_entries=df_lenght, title="Disk usage")
    ...
    build_plot_nodes(list(timestamps[:len_lists]), df_list=list_of_cpus, num_entries=len_lists, colors=colors, metric_name="CPU")
    plt.savefig(plots_pathname + 'cpu_nodes.png')
    plt.close()

    build_plot_nodes(list(timestamps[:len_lists]), df_list=list_of_mems, num_entries=len_lists, colors=colors, metric_name="memory")
