		...
		for (int i = 0; i < MSIZE; ++i) {
			for (int j = 0; j < MSIZE; ++j) {
				for (int block = 0; block < BSIZE*BSIZE; ++block) {
					if (tag.equals("A")) A[i][j][block] = (Math.random())*10.0;
					else B[i][j][block] = (Math.random())*10.0;
				}
			}
		}
	}
		...
		try {
			...
			for (int i = 0; i < MSIZE; ++i) {
				for (int j = 0; j < MSIZE; ++j) {
					for (int block = 0; block < BSIZE*BSIZE; ++block) {
						String value;
						if (tag.equals("A")) value = String.valueOf(A[i][j][block]) + " ";
						else if (tag.equals("B")) value = String.valueOf(B[i][j][block]) + " ";
						else value = String.valueOf(C[i][j][block]) + " ";
