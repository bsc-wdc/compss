...
public class SimpleElasticity {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            // Poll until minVM initial VMs are ready
            ...
            // Sync results and verify correctness; at i==2 poll for maxVM elastic VMs
            for (int i = 0; i < increment; i++) {
                ...
                if (i == 2) {
                    System.out.println("[LOG] Waiting for elastic VMs (" + maxVM + ")...");
                    waitForWorkerCount(maxVM, creationTime * 2000L, "Intermediate VM creation");
                }
            }
            ...
            // Poll until the scheduler has scaled back to minVM, then pause briefly
            // so the ResourceManager's applyPolicies cycle logs currentVMs==minVM
            // before the application exits (result script checks the last logged value).
