...
def compss_worker(persistent_storage):
    ...
    try:
        ...
        if persistent_storage:
            with storage_task_context(logger, values, config_file_path=storage_conf):
                getattr(module, method_name)(*values, compss_types=types, compss_tracing=tracing, compss_return_length=num_returns)
                if tracing:
    ...
    # ==========================================================================
        ...
        if has_target == 'true':
            ...
            if persistent_storage:
                with storage_task_context(logger, values, config_file_path=storage_conf):
                    getattr(klass, method_name)(*values, compss_types=types, compss_tracing=tracing, compss_return_length=num_returns)
                    if tracing:
                        pyextrae.eventandcounters(TASK_EVENTS, 0)
                        pyextrae.eventandcounters(TASK_EVENTS, WORKER_END)
            else:
