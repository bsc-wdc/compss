...
public class ITAppModifier {
	...
	/*
	 * Flag to instrument main method. if IT_IS_MAINCLASS mainclass not defined (Default case) isMain gets true;
	 */
			...
			if (LoaderUtils.checkRemote(m, remoteMethods) == null) {
				...
				if (isMainProgram && isMainClass) {
					...
					if (isWSClass) { //
						...
						m.insertBefore(toInsertBefore.toString());
						// TODO remove old code m.insertAfter(toInsertAfter.toString()); // executed only if
						// Orchestration finishes properly
					}
					m.instrument(converter);
					m.instrument(itAppEditor);
				} else {
	...
	private void manageStartAndStop(CtClass appClass, String itApiVar, String itSRVar, String itORVar) throws CannotCompileException,
			NotFoundException {
		...
		toInsertBefore
				.append(itApiVar + " = new " + LoaderConstants.CLASS_COMPSS_API_IMPL + "();")
				...
				.append(itORVar + " = new " + LoaderConstants.CLASS_OBJECT_REGISTRY + "((" + LoaderConstants.CLASS_LOADERAPI + ") "
						+ itApiVar + " );").append(itApiVar + ".startIT();");
